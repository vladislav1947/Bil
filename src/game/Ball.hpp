...
public:
    ...
    void startFalling(const glm::vec3& pocketDirection);
...
void Ball::update(float deltaTime) 
{
    ...
    if (isFalling) {
        ...
        fallTime += deltaTime;
        velocity.y -= 9.8f * deltaTime * 0.5f; 
        velocity *= (1.0f - 0.2f * deltaTime);
        ...
        if (position.y < -2.0f) { // -2.0f - уровень "исчезновения"
            isPocketed = true;
...
void Ball::startFalling(const glm::vec3& pocketDirection)
    {
        ...
        velocity = glm::normalize(pocketDirection) * 0.5f;
        velocity.y = 0.3f; // Небольшой подскок
    }
